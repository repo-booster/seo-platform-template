<template>
  <div class="space-y-8 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="text-center max-w-3xl mx-auto">
      <h1 class="text-3xl font-bold mb-4">Plans & Pricing</h1>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
      <div v-for="plan in plans" :key="plan.name" 
           class="bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow p-6 border"
           :class="{ 'border-blue-500': plan.popular, 'border-gray-200': !plan.popular }">
        <div class="text-center">
          <h2 class="text-xl font-semibold">{{ plan.name }}</h2>
          <div class="mt-4">
            <span class="text-4xl font-bold">${{ isAnnual ? plan.annualPrice : plan.monthlyPrice }}</span>
